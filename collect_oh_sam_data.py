			...
			if ("** WRITER_" in line) or (">>>>" in line) or ("** READER_" in line) or ("** SERVER_" in line):
				...
				if("** READER_" in line):
					...
					if(protocol==1) or (protocol==6) or (protocol==11) or (protocol==16) or (protocol==21): #ABD
						...
						r_fastReads += 0
					elif (protocol==2) or (protocol==7) or (protocol==12) or (protocol==17) or (protocol==22) or (protocol==33) or (protocol==34): #ohSAM
						r_slowReads += int(line.split("3EXCH_reads=")[1].split(",")[0])
						r_fastReads += 0
					elif (protocol==5) or (protocol==10) or (protocol==15) or (protocol==20) or (protocol==25) or (protocol==31) or (protocol==32): #ohFast #ohSamEX
						r_slowReads += int(line.split("3EXCH_reads=")[1].split(",")[0])
						r_fastReads += int(line.split("2EXCH_reads=")[1].split(",")[0])
					else:
						r_slowReads += int(line.split("4EXCH_reads=")[1].split(",")[0])  #Hybrid #Semifast
						r_fastReads += int(line.split("2EXCH_reads=")[1].split(",")[0])
	...
	main_directory = create_output_file_for_scenario(main_results_dir,0)
		...
		for protocol in range(prtcl_start,prtcl_stop+1,prtcl_step):
                    ...
                    if (protocol==1):
                        ...
                        alg=oh_samEX+"-p2p"
                        executable="am-ohSamEX-p2p"
                    elif (protocol==32):
                        alg=oh_samEX+"-star-p2p"
                        executable="am-ohSamEX-star-p2p"
                    elif (protocol==33):
                        alg=oh_sam+"-star-p2p"
                        executable="am-ohSam-star-p2p"
                    elif (protocol==34):
