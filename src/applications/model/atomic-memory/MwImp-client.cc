...
NS_LOG_COMPONENT_DEFINE ("MwImpClientApplication");

NS_OBJECT_ENSURE_REGISTERED (MwImpClient);


void
MwImpClient::LogInfo( std::stringstream& s)
{
...
TypeId
MwImpClient::GetTypeId (void)
{
  //static vector<Address> tmp_address;
  static TypeId tid = TypeId ("ns3::MwImpClient")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<MwImpClient> ()
    .AddAttribute ("MaxOperations",
                   "The maximum number of operations to be invoked",
                   UintegerValue (100),
                   MakeUintegerAccessor (&MwImpClient::m_count),
                   MakeUintegerChecker<uint32_t> ())
	.AddAttribute ("MaxFailures",
					  "The maximum number of server failures",
					  UintegerValue (100),
					  MakeUintegerAccessor (&MwImpClient::m_fail),
					  MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Interval", 
                   "The time to wait between packets",
                   TimeValue (Seconds (1.0)),
                   MakeTimeAccessor (&MwImpClient::m_interval),
                   MakeTimeChecker ())
   .AddAttribute ("SetRole",
					  "The role of the client (reader/writer)",
					  UintegerValue (0),
					  MakeUintegerAccessor (&MwImpClient::m_prType),
					  MakeUintegerChecker<uint16_t> ())
   .AddAttribute ("LocalAddress",
					  "The local Address of the current node",
					  AddressValue (),
					  MakeAddressAccessor (&MwImpClient::m_myAddress),
					  MakeAddressChecker ())
	.AddAttribute ("RemoteAddress",
                   "The destination Address of the outbound packets",
                   AddressValue (),
                   MakeAddressAccessor (&MwImpClient::m_peerAddress),
                   MakeAddressChecker ())
    .AddAttribute ("RemotePort", 
                   "The destination port of the outbound packets",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MwImpClient::m_peerPort),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("Port", "Port on which we listen for incoming packets.",
					UintegerValue (9),
					MakeUintegerAccessor (&MwImpClient::m_port),
					MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("PacketSize", "Size of echo data in outbound packets",
                   UintegerValue (100),
                   MakeUintegerAccessor (&MwImpClient::SetDataSize,
                                         &MwImpClient::GetDataSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Tx", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&MwImpClient::m_txTrace),
                     "ns3::Packet::TracedCallback")
    .AddAttribute ("ID", 
                     "Client ID",
                   	 UintegerValue (100),
                  	 MakeUintegerAccessor (&MwImpClient::m_personalID),
                  	 MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Clients", 
                     "Number of Clients",
                   	 UintegerValue (100),
                  	 MakeUintegerAccessor (&MwImpClient::m_numClients),
                  	 MakeUintegerChecker<uint32_t> ())
	 .AddAttribute ("RandomInterval",
					 "Apply randomness on the invocation interval",
					 UintegerValue (0),
					 MakeUintegerAccessor (&MwImpClient::m_randInt),
					 MakeUintegerChecker<uint16_t> ())
	 .AddAttribute ("Seed",
					 "Seed for the pseudorandom generator",
					 UintegerValue (0),
					 MakeUintegerAccessor (&MwImpClient::m_seed),
					 MakeUintegerChecker<uint16_t> ())
	.AddAttribute ("Verbose",
					 "Verbose for debug mode",
					 UintegerValue (0),
					 MakeUintegerAccessor (&MwImpClient::m_verbose),
					 MakeUintegerChecker<uint16_t> ())
  ;
  return tid;
}
 ...
 **************************************************************************************/
void 
MwImpClient::StartApplication (void)
{
	...
	m_insocket->SetRecvCallback (MakeCallback (&MwImpClient::HandleRecv, this));

	// Accept new connection
	m_insocket->SetAcceptCallback (
			MakeNullCallback<bool, Ptr<Socket>, const Address &> (),
			MakeCallback (&MwImpClient::HandleAccept, this));
	// Peer socket close handles
	m_insocket->SetCloseCallbacks (
			MakeCallback (&MwImpClient::HandlePeerClose, this),
			MakeCallback (&MwImpClient::HandlePeerError, this));
	//////////////////////////////////////////////////////////////////////////////
	...
	if ( m_socket.empty() )
	{
		...
		for (uint32_t i = 0; i < m_serverAddress.size(); i++ )
		{
			...
			m_socket[i]->SetRecvCallback (MakeCallback (&MwImpClient::HandleRecv, this));
			m_socket[i]->SetAllowBroadcast (false);

			m_socket[i]->SetConnectCallback (
				        MakeCallback (&MwImpClient::ConnectionSucceeded, this),
				        MakeCallback (&MwImpClient::ConnectionFailed, this));
		}
	}
 ...
 * Connection handlers
 **************************************************************************************/
void MwImpClient::HandlePeerClose (Ptr<Socket> socket)
{
	NS_LOG_FUNCTION (this << socket);
}

void MwImpClient::HandlePeerError (Ptr<Socket> socket)
{
	NS_LOG_FUNCTION (this << socket);
}


void MwImpClient::HandleAccept (Ptr<Socket> s, const Address& from)
{
	NS_LOG_FUNCTION (this << s << from);
	s->SetRecvCallback (MakeCallback (&MwImpClient::HandleRecv, this));
	m_socketList.push_back (s);
}



void MwImpClient::ConnectionSucceeded (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address from;
...
void MwImpClient::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  std::stringstream sstm;
 ...
 **************************************************************************************/
void
MwImpClient::SetServers (std::vector<Address> ip)
{
	m_serverAddress = ip;
...
void
MwImpClient::SetRemote (Address ip, uint16_t port)
{
 ...
 **************************************************************************************/
void 
MwImpClient::SetDataSize (uint32_t dataSize)
{
...
uint32_t 
MwImpClient::GetDataSize (void) const
{
  NS_LOG_FUNCTION (this);
  return m_size;
}
...
void 
MwImpClient::SetFill (std::string fill)
{
 ...
 */
void 
MwImpClient::ScheduleOperation (Time dt)
{
  ...
  if (m_prType == READER )
  {
	  m_sendEvent = Simulator::Schedule (dt, &MwImpClient::InvokeRead, this);
  }
  else
  {
	  m_sendEvent = Simulator::Schedule (dt, &MwImpClient::InvokeWrite, this);
  }
}

/**************************************************************************************
 * MwImp Read/Write Handlers
 **************************************************************************************/
void
MwImpClient::InvokeRead (void)
{
	NS_LOG_FUNCTION (this);
	std::stringstream sstm;
	m_opStart = Now();
...
MwImpClient::InvokeWrite (void)
{
	...
	if ( m_opCount <=  m_count )
	{
		...
		AsmCommon::Reset(sstm);
		sstm << "** WRITE INVOKED: " << m_personalID << " at "<< m_opStart.GetSeconds() <<"s";
		LogInfo(sstm);
		HandleSend();
	}
}
...
MwImpClient::HandleSend (void)
{
  ...
  if (m_dataSize)
    {
      NS_ASSERT_MSG (m_dataSize == m_size, "MwImpClient::HandleSend(): m_size and m_dataSize inconsistent");
      NS_ASSERT_MSG (m_data, "MwImpClient::HandleSend(): m_dataSize but no m_data");
...
void
MwImpClient::HandleRecv (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
MwImpClient::ProcessReply(uint32_t type, uint32_t ts, uint32_t id, uint32_t val, uint32_t op)
{
	NS_LOG_FUNCTION (this);
	...
	if (m_prType == WRITER)
	{
      	...
      	if (m_replies >= (m_numServers - m_fail))
      	{
			...
			ScheduleOperation (m_interval);
      		AsmCommon::Reset(sstm);
			sstm << "** WRITE COMPLETED: "  << m_opCount << " in "<< ((m_opEnd.GetSeconds() - m_opStart.GetSeconds()) + elapsed_seconds.count()) << "s (<"<<(m_opEnd.GetSeconds() - m_opStart.GetSeconds())<<"> + <"<< elapsed_seconds.count() <<">) - @ 3 EXCH **";
			LogInfo(sstm);
			m_replies =0;
		}
	}
	...
	{	////////////////////////////////////////////////
		...
		////////////////////////////////////////////////
      	...
      	if (m_replies >= (m_numServers - m_fail))
      	{
			...
			ScheduleOperation (m_interval);
      		AsmCommon::Reset(sstm);
			sstm << "** READ COMPLETED: "  << m_opCount << " in "<< ((m_opEnd.GetSeconds() - m_opStart.GetSeconds()) + elapsed_seconds.count()) << "s (<"<<(m_opEnd.GetSeconds() - m_opStart.GetSeconds())<<"> + <"<< elapsed_seconds.count() <<">) - @ 3 EXCH **";
			LogInfo(sstm);
			m_replies =0;
		}
	}
