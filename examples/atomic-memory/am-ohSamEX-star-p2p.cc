...
NS_LOG_COMPONENT_DEFINE ("ohSamEXExample");
...
main (int argc, char *argv[])
{
    ...
    //
#if 1
    LogComponentEnable ("ohSamEXExample", LOG_LEVEL_INFO);
    LogComponentEnable ("ohSamEXClientApplication", LOG_LEVEL_INFO);
    LogComponentEnable ("ohSamEXServerApplication", LOG_LEVEL_INFO);
#endif
    //
    ...
    //
    // Create a ohSamEXServer application on node one.
    //
    ...
    for (int i=0; i<numServers; i++)
    {
        ohSamEXServerHelper server (port);
    ...
    //
    // Create a ohSamEXClient application to send UDP datagrams from node zero to
    // node one.
    //
    ...
    for (int i=0; i<numClients; i++)
    {
        ...
        ohSamEXClientHelper client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
    ...
    //csma.EnableAsciiAll (ascii.CreateFileStream ("am-ohSamEX.tr"));
    //csma.EnablePcapAll ("am-ohSamEX", false);
    ...
    //
    NS_LOG_INFO ("Run Simulation: oh-SamEx.");
    //std::cout<<"Run Simulation: oh-Sam SPIKE."<<std::endl;
    Simulator::Run ();
    Simulator::Destroy ();
    NS_LOG_INFO (">>>> oh-SamEX Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:1, Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
