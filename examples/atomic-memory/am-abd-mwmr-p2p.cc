...
NS_LOG_COMPONENT_DEFINE ("ABDMWMRExample");
...
//
#if 1
  LogComponentEnable ("ABDMWMRExample", LOG_LEVEL_INFO);
  LogComponentEnable ("AbdServerMWMRApplication", LOG_LEVEL_INFO);
  LogComponentEnable ("AbdClientMWMRApplication", LOG_LEVEL_INFO);
#endif
//
...
//
// Create a ABDMWMRServer application on node one.
//
...
//
// Create a ABDMWMRClient application to send UDP datagrams from node zero to
// node one.
//
  ...
  for (int i=0; i<numClients; i++)
  {
      ...
      AbdClientHelperMWMR client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
  ...
  //csma.EnableAsciiAll (ascii.CreateFileStream ("am-ABDMWMR.tr"));
  //csma.EnablePcapAll ("am-ABDMWMR", false);
...
//
  NS_LOG_INFO ("Run Simulation: ABD MWMR p2p.");
  //std::cout<<"Run Simulation: oh-Sam p2p."<<std::endl;
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO (">>>> ABDMWMR p2p Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:"<<numWriters<<", Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
