...
//
  ...
  for (int i=0; i<numClients; i++)
  {
      ...
      ohSamClientHelper client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
...
//
  NS_LOG_INFO ("Run Simulation: oh-SamEX p2p.");
  //std::cout<<"Run Simulation: oh-Sam SPIKE."<<std::endl;
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO (">>>> oh-SamEX p2p Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:1, Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
