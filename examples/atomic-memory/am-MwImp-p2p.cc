...
NS_LOG_COMPONENT_DEFINE ("MwImpExample");
...
//
#if 1
  LogComponentEnable ("MwImpExample", LOG_LEVEL_INFO);
  LogComponentEnable ("MwImpClientApplication", LOG_LEVEL_INFO);
  LogComponentEnable ("MwImpServerApplication", LOG_LEVEL_INFO);
#endif
//
...
//
// Create a MwImpServer application on node one.
//
...
//
// Create a MwImpClient application to send UDP datagrams from node zero to
// node one.
//
  ...
  for (int i=0; i<numClients; i++)
  {
      ...
      MwImpClientHelper client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
  ...
  //csma.EnableAsciiAll (ascii.CreateFileStream ("am-MwImp.tr"));
  //csma.EnablePcapAll ("am-MwImp", false);
...
//
  NS_LOG_INFO ("Run Simulation: MW Impossibility p2p.");
  //std::cout<<"Run Simulation: oh-Sam p2p."<<std::endl;
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO (">>>> MW Impossibility Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:"<<numWriters<<", Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
