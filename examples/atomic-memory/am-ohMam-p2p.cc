...
//
  ...
  for (int i=0; i<numClients; i++)
  {
      ...
      OhMamClientHelper client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
...
//
  NS_LOG_INFO ("Run Simulation: oh-Mam p2p.");
  //std::cout<<"Run Simulation: oh-Sam p2p."<<std::endl;
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO (">>>> oh-Mam Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:"<<numWriters<<", Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
