...
NS_LOG_COMPONENT_DEFINE ("OhMamEXExample");
...
//
#if 1
  LogComponentEnable ("OhMamEXExample", LOG_LEVEL_INFO);
  LogComponentEnable ("OhMamEXClientApplication", LOG_LEVEL_INFO);
  LogComponentEnable ("OhMamEXServerApplication", LOG_LEVEL_INFO);
#endif
//
...
//
  ...
  /********************************************************************
      ...
      writerNode.Create(numWriters); /////Fanos: it was 1
...
//
// Create a OhMamEXServer application on node one.
//
  ...
  for (int i=0; i<numServers; i++)
  {
	  OhMamEXServerHelper server (port);
...
//
// Create a OhMamEXClient application to send UDP datagrams from node zero to
// node one.
//
  ...
  for (int i=0; i<numClients; i++)
  {
      ...
      OhMamEXClientHelper client (Address(p2pClientsInterfaceAdjacencyList[i].GetAddress (1)), port);
  ...
  //csma.EnableAsciiAll (ascii.CreateFileStream ("am-OhMamEX.tr"));
  //csma.EnablePcapAll ("am-OhMamEX", false);
...
//
  NS_LOG_INFO ("Run Simulation: oh-MamEX p2p.");
  //std::cout<<"Run Simulation: oh-Sam p2p."<<std::endl;
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO (">>>> oh-MamEX Scenario - Servers:"<<numServers<<", Readers:"<<numReaders<<", Writers:"<<numWriters<<", Failures:"<<numFail<<", ReadInterval:"<<readInterval<<", WriteInterval:"<<writeInterval<<", <<<<");
