...
NS_LOG_COMPONENT_DEFINE ("CCHybridExample");
...
//
#if 1
  LogComponentEnable ("CCHybridExample", LOG_LEVEL_INFO);
  LogComponentEnable ("CCHybridClientApplication", LOG_LEVEL_INFO);
  LogComponentEnable ("CCHybridServerApplication", LOG_LEVEL_INFO);
#endif
//
...
//
// Create a CCHybridClient application to send UDP datagrams from node zero to
// node one.
//
  ...
  // Create the writer+reader processes
  	...
  	for (int i=0; i< numClients; i++)
  	{
  		...
  		CCHybridClientHelper client (Address(csmaInterfaceAdjacencyList[lan].GetAddress ((i%clientsPerLan)+2)), port);
