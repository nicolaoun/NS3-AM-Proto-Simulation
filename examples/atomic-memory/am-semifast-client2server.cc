...
main (int argc, char *argv[])
{
    ...
    ********************************************************************/
    ...
    std::vector<NodeContainer> clientServerAdjacencyList;
    ...
    for(int i=0; i<numClients; i++)
    {
        for(int j=0; j<numServers; j++)
        {
            ...
            clientServerAdjacencyList.push_back( NodeContainer (clientNodes.Get(i), serverNodes.Get(j)) );
        }
    }

    int nSpokes = numServers + numClients;
    ...
    for(uint32_t i=0; i<clientServerAdjacencyList.size (); ++i)
    {
        p2pDeviceAdjacencyList.push_back( p2p.Install (clientServerAdjacencyList[i]) );
    }
    ...
    std::vector<Address> tempAddress;
    for(uint32_t i=0; i<p2pDeviceAdjacencyList.size (); ++i)
    {
        ...
        tempAddress.push_back(p2pInterfaceAdjacencyList[i].GetAddress(0));

        // every S links store the server addresses for client
        if ( i%(numServers-1) == 0)
        {
            serverAddress.push_back(tempAddress);
            tempAddress.clear();
        }
    }
    ...
    for (int i=0; i<	numServers; i++)
    {
        ...
        server.SetAttribute("LocalAddress", AddressValue (p2pInterfaceAdjacencyList[i].GetAddress(0)) );
